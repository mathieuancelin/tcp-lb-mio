...
impl Proxy {
    ...
    fn accept_one(&mut self, event_loop: &mut EventLoop<Proxy>) -> bool {
        ...
        match client_sock.peer_addr() {
            Ok(client_addr) => debug!("New client connection from {}", client_addr),
            Err(_) => debug!("New client connection from unknown source"),
        }
...
impl Drop for Connection {
    fn drop(&mut self) {
        debug!("Dropping Connection with {:?}", self.token);
    }
}
